...
/**
 * 동기 Spring Event 구조에서도 추천 대상 조회가 태그 일치 공연자 ID로
 * MemberLike 조회 범위를 먼저 제한하고 개인화 메시지를 생성하는지 검증한다.
 */
...
class NoticeRecommendationServiceTest {
    ...
    @Mock private AmateurTicketRepository amateurTicketRepository;
    @Mock private MemberLikeRepository memberLikeRepository;
    @Mock private AmateurShow newShow;
    @Mock private Member performer;
    ...
    @Test
    void narrowsLikesByMatchingPerformerAndCreatesPersonalizedNotice() {
        ApproveShowEvent event = new ApproveShowEvent(newShow, performer);
